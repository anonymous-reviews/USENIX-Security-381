...
{
  // C8. Global variable issues - 0x994 instead of &decoding
  // C8. Global variable issues - &operand_write instead of &decoding
  // C1.a.i. Pointer arithmetic to access struct members
  // C3.a. - Two extraneous type casts
  interpret_rtl_and(&t0, (_DWORD *) 0x994, ((_DWORD *) (&operand_write)) + 1);
  rtl_update_ZFSF(&t0, decoding[19]); // C8., C1.a.ii.
  interpret_rtl_li(&t1, 0);
  rtl_set_CF(&t1);
  return rtl_set_OF(&t1); // C12.a.
}
...
{
  // C2.b. Missing statement - Missing variable: io = ios
  long long v2;
...
long long vdp_data_read()
{
  ...
  vdp_data_read_ahead();
  return v0; // I'm not sure if this should be coded or not, but an unsigned char is returned as a long long here, but the original function returns a uint8_t.
}
...
{
  signed long long result; // C5. Extraneous variables - For return in void func.  It seems like this should be a specific subclass.
  E = 0; // C13., C1.a. Non-idiomatic dereference - Not a pointer type.  Maybe should be a new subtype.
  dword_4040E4 = 0; // C8., C13.
  dword_4040F0 = 0; // C8., C13.
  result = getWindowSize(&dword_4040E8, &dword_4040EC); // C8., C13, C4. - Assignment to extraneous variable
  if (((_DWORD) result) == (-1)) // C3.a. Extraneous typecast
    die("getWindowSize");
...
{
  long long result; // C5. Extraneous variables - variable for extraneous return
  // C3.a. Extraneous typecasts
  // C1.a.i. Pointer arithmetic to access struct members
  *((_BYTE *) (a1 + 133)) = 1;
...
long long rol_a()
{
  ...
  if (P & 1)
  {
    if (A >= 0) // Equivalent bitwise expression
      P &= 0xFEu; // C6. Non-idiomatic literal representation - Original code had ~0x1 instead of 0xFEu.
    else
      P |= 1u;

    A = (2 * A) | 1; // Multiplication instead of shift.  I think this is OK not to code.
  }
  else
  {
    if (A >= 0) // Equivalent bitwise expression
    ...
    A *= 2; // Multiplication instead of shift.  I think this is OK not to code.
  }

  if (A)
    P &= 0xFDu; // C6. Non-idiomatic literal representation - Original code had ~0x2 instead of 0xFDu.
  else
    P |= 2u;

  if (A >= 0) // Equivalent bitwise expression
    P &= 0x7Fu; // C6. Non-idiomatic literal representation - Original code had ~0x80 instead of 0x7Fu.
  else
    P |= 0x80u;
...
{
  fprintf(_bss_start, "%s [--unit <B|M|G|T>] <DEVICE> [<DEVICE> ...]\n", a1); // C8., C11. - C11 may be a stretch, but most likely stdout and _bss_start are both symbols for the same address.  But stdout is a standard one, _bss_start is not.
  exit(0);
}
...
{
  long long result;
  unsigned int v6; // C5.b. Extraneous variable duplicating another variable - a5
  long long v7; // C5.b. Extraneous variable duplicating another variable - a4
  v7 = a4; // C2.a.i.
  v6 = a5; // C2.a.i.
  if (!strcmp(a3, "format")) 
    result = fz_strlcpy(v7, "CBZ", v6);
  else
    result = 0xFFFFFFFFLL; // C6.c. Very large positive numbers for negative integers
...
{
  int *v0; // C5. Extraneous variables
  char *v1; // C5. Extraneous variables - ternary temporary
  if (((char *) list_remove_value()) == test1) // C3.a. Extraneous typecast
    return 0LL;

  if (*__errno_location()) // C9.a. Expanded standard macro, C7.d. Deconstructed ternary
  {
    v0 = __errno_location(); // C2.a.i.
    v1 = strerror(*v0); // C2.a.i.
  }
  else
  {
    v1 = "None"; // C2.a.i.
...
_BOOL8 confirm_delete()
{
  ...
  do
  {
    ...
    // C8. Global variable issue
    printf(&byte_405FB0);

    // C4.a.ii. Missing arguments
    // C4. - scanf vs __iso99_scanf.  This is really a compiler change, so I'm no sure if we want to code it as a decompiler problem.
    __isoc99_scanf();
  }
  while (v1 && (v1 != 1));
  return v1 == 0;
...
{
  int result;
  unsigned char v4; // C5.b. Extraneous variable duplicating another variable
  long long v5;
...
{
  long long v2;
  unsigned long long v3; // C5. Extraneous variable
  v3 = __readfsqword(0x28u); // C2.a.i. Extraneous statement to initialize extraneous variable
  v2 = qemu_coroutine_self();
  // C1.a.i. Pointer arithmetic to access struct members
  *((_QWORD *) (v2 + 32)) = *a1;
  // C1.a.i. Pointer arithmetic to access struct members
  if (*((_QWORD *) (v2 + 32)))
  // C1.a.i. Pointer arithmetic to access struct members - (two)
    *((_QWORD *) ((*a1) + 40)) = v2 + 32;
  else
...
{
  signed long long result; // C5. Extraneous variable
  // C2.b. Missing statement - assert

  // C9.b. Expanded user-defined macros (three)
  if ((a2 && (a2 <= 0x1B5D)) || ((a2 > 0x1F40) && (a2 <= 0x1F85)))
    // C9.b. Expanded user-defined macro
...
{
  _DWORD *v1;
  _DWORD *v2; // Extraneous variable duplicating another variable
  v1 = malloc(0x20020uLL);
  v2 = v1; // C2.a.i. Extra statement to initialize extraneous variable
  v1[32775] = a1; // C1.a.ii. Array access to access struct member
  v1[2] = 0; // C1.a.ii. Array access to access struct member
  v1[3] = 0; // C1.a.ii. Array access to access struct member
...
long long namcos2_sound_bankselect_w(long long a1, unsigned char a2)
{
  long long v2; // RAM
  long long result; // C5. Extraneous variable
  unsigned int v4; // C5. Extraneous variable
  v2 = memory_region(131LL); // C9.b. Expanded user-defined macros

  // C2.b. Missing statement - unsigned long max = ...
  // C2.b. Missing statement - int bank = ...
  // C2.b. Missing statement - if ((6 >- 1)) ...

   // C9.b. Expanded user-defined macros
   // C3.a. Extraneous typecasts
  cpu_bankbase[6] = v2 + ((signed int) ((((a2 >> 4) % (((unsigned long long) (memory_region_length(131LL) - 0x10000)) >> 14)) + 4) << 14));
  result = opcode_entry; // C2.a.i. Extraneous statement to initialize extraneous variable
  if (opcode_entry == 6)
  {
    result = cpu_getactivecpu(); // C2.a.i. Extraneous statement to initialize extraneous variable
    if (((signed int) result) >= 0) // C3.a. Extraneous typecast
    {
      opcode_entry = -1; // C6. Negative integer used for large positive integer!
      v4 = activecpu_get_pc_byte(); // C2.a.i. Extraneous statement to initialize extraneous variable
      result = activecpu_set_op_base(v4); // C4. Nonequivalent expression - write to extraneous variable
    }
...
void ft_errorlog(long long a1)
{
  ...
  if (a1)
  {
    // C9.a. Expanded standard macro - O_CREAT, O_WRONLY, O_APPEND, S_IRWXU
    fd = open("/tmp/.sh_errorlog", 1089, 448LL);
    if (fd >= 0)
    {
      // C3.b. Extraneous typecast
...
{
  long long result; // C5.b. Extraneous variable duplicating another variable
  result = a4; // C2.a.i. Extra statement to initialize extraneous variable
  // C3.a. Extraneous typecast
  // C1.a.i. Pointer arithmetic to access struct members
  // C1.b. Pointer arithmetic to access array members (list[pos])
...
int main(int argc, const char **argv, const char **envp)
{
  ...
  // C3.a. Extraneous typecasts
  test_machine_init(*((_QWORD *) (&argc)), argv, envp);
  test_machine_command_create();
  test_machine_command_with_clamp();
  test_machine_command_valid_check();
  // C4.b. Equivalence depends on behavior of external code - Compiler switched printf + newline to puts.
